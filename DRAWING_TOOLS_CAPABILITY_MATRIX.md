...
| Public APIs | Shared command/API concepts exist with explicit changed/no-op returns for no-op-capable commands | Web API coverage broad, including boolean returns for active-tool, selection, clear, and cancel-draft commands | Mobile handle coverage broad, including matching boolean returns for active-tool, selection, clear, and cancel-draft commands | `partial` | Gap 8 polish |
