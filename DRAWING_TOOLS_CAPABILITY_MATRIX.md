...
| Public APIs | Shared command/API concepts exist with explicit changed/no-op returns for no-op-capable commands | Web API coverage broad, including boolean returns for active-tool, selection, clear, cancel-draft, state replacement, and layout import paths | Mobile handle coverage broad, including matching boolean returns for active-tool, selection, clear, cancel-draft, state replacement, and layout import paths | `partial` | Gap 8 polish |
