...
The priority for the next drawing-tools push is visible TradingView-grade
drawing UX: real placement behavior, local selected actions, context/object-tree
workflow, edit lifecycle polish, and undo/redo confidence. Isolated
primitive/export hardening should only happen when it directly unblocks one of
those UX epics.

When adding new drawing shapes, keep interaction, command, history, toolbar,
context-menu, and object-tree gaps visible so shared behavior and parity gates
stay consistent across web Canvas and mobile Skia.
...
- Anchor-based web Canvas and mobile Skia drawing input use the same weak/strong
...
- Optional normalized pressure metadata is now preserved on freehand anchors
  ...
  segments from that pressure metadata at render time, including dash-phase
  continuity for dashed and dotted strokes.
...
- Brush drag samples preserve optional pressure metadata on web and mobile, and
  ...
  path-family segment model with dash-phase continuity.
...
## Shipped: Volume Profile Row Count Controls

- Shared drawing style now preserves bounded `volumeProfileRowCount` settings
  for anchored and fixed-range volume profile drawings.
- Web selected-drawing properties panels and mobile Skia properties sheets
  expose matching 12/24/48 row presets through the shared properties surface.
...
## Shipped: Risk/Reward Label Alignment Controls

- Shared drawing style now preserves `riskRewardLabelAlignment` for long and
  short position drawings with center as the default and left/right as alternate
