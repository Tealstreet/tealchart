...
describe('user drawing renderer', () => {
  ...
  it('renders price range labels at the configured horizontal alignment', () => {
    const ctx = new RecordingCanvasContext();
    const drawing: UserDrawing = {
      ...base,
      id: 'range',
      kind: 'priceRange',
      style: { ...base.style, measurementLabelAlignment: 'right' },
      points: [
        { time: 10, price: 10 },
        { time: 90, price: 90 },
      ],
  ...
  it('renders date and price range labels at the configured horizontal alignment', () => {
    const ctx = new RecordingCanvasContext();
    const drawing: UserDrawing = {
      ...base,
      id: 'date-price-range',
  ...
  it('renders bars pattern candles through CanvasContext', () => {
    ...
    const drawing: UserDrawing = {
      ...
      id: 'bars',
      kind: 'barsPattern',
      style: { ...base.style, barsPatternUpColor: '#38bdf8', barsPatternDownColor: '#f97316' },
      points: [
        { time: 10, price: 50 },
        { time: 20, price: 50 },
        { time: 40, price: 50 },
      ...
      bars: [
        ...
        { time: 20, open: 53, high: 58, low: 51, close: 52 },
      ],
    };
    ...
    expect(ctx.calls).toContain('fillRect:36.5,50,7,2:#38bdf8:1');
    expect(ctx.calls).toContain('strokeRect:36.5,50,7,2:#f5c542:1');
    expect(ctx.calls).toContain('fillRect:46.5,49,7,1:#f97316:1');
  });
  ...
  it('renders XABCD pattern drawings as labeled stroked polylines', () => {
    const ctx = new RecordingCanvasContext();
    const drawing: UserDrawing = {
      ...base,
      id: 'xabcd',
    ...
    expect(ctx.calls).toContain('fillText:B:50,4:#111:center:1:12px sans-serif');
  ...
  it('renders fib wedges as filled Fibonacci arc wedges', () => {
    ...
    expect(ctx.calls).toContain('fill');
    expect(ctx.calls).toContain('moveTo:10,50');
    expect(ctx.calls).toContain('lineTo:60,50');
    expect(ctx.calls).toContain('lineTo:50,50');
  ...
  it('renders gann boxes as filled grid and angle boxes', () => {
    ...
    expect(ctx.calls).toContain('fillRect:10,20,80,60:rgba(245, 197, 66, 0.12):1');
    expect(ctx.calls).toContain('moveTo:10,50');
    expect(ctx.calls).toContain('lineTo:90,50');
    expect(ctx.calls).toContain('moveTo:50,20');
    expect(ctx.calls).toContain('lineTo:50,80');
  ...
  it('renders sine lines as sampled wave paths', () => {
    ...
    expect(ctx.calls).toContain('lineTo:10,50');
    expect(ctx.calls).toContain('lineTo:20,20');
    expect(ctx.calls).toContain('stroke:#f5c542:2:6,4:1');
  });
  ...
  it('renders fixed range volume profiles as shared volume bins', () => {
    const ctx = new RecordingCanvasContext();
    const drawing: UserDrawing = {
      ...base,
      id: 'volume-profile',
    ...
    expect(ctx.calls).toContain('fillRect:10,20,80,5:rgba(245, 197, 66, 0.12):1');
  ...
  it('can hide fixed range volume profile guides while preserving bins and bounds', () => {
    const ctx = new RecordingCanvasContext();
    const drawing: UserDrawing = {
      ...base,
      id: 'volume-profile',
    ...
    expect(ctx.calls).toContain('fillRect:10,20,80,5:rgba(245, 197, 66, 0.12):1');
    expect(ctx.calls).toContain('strokeRect:10,20,80,60:#f5c542:1');
    expect(ctx.calls).not.toContain('moveTo:10,22.5');
    expect(ctx.calls).not.toContain('lineTo:90,22.5');
    expect(ctx.calls).not.toContain('stroke:#f5c542:2::1');
  ...
  it('renders fixed range volume profile guides with configured value areas', () => {
    const ctx = new RecordingCanvasContext();
    const drawing: UserDrawing = {
      ...base,
      id: 'volume-profile',
    ...
    expect(ctx.calls).toContain('moveTo:10,20');
    expect(ctx.calls).toContain('lineTo:90,20');
    expect(ctx.calls).toContain('moveTo:10,25');
    expect(ctx.calls).toContain('lineTo:90,25');
    expect(ctx.calls).not.toContain('lineTo:90,45');
  });

  it('renders fixed range volume profiles with configured profile widths', () => {
    const ctx = new RecordingCanvasContext();
    const drawing: UserDrawing = {
      ...base,
      id: 'volume-profile',
    ...
    expect(ctx.calls).toContain('fillRect:10,20,40,5:rgba(245, 197, 66, 0.12):1');
    expect(ctx.calls).toContain('fillRect:10,40,20,5:rgba(245, 197, 66, 0.12):1');
    expect(ctx.calls).toContain('lineTo:50,22.5');
    expect(ctx.calls).not.toContain('lineTo:90,22.5');
  });
  ...
  it('renders note drawings with measured boxes', () => {
    const ctx = new RecordingCanvasContext();
    const drawing: UserDrawing = {
      ...base,
      id: 'note',
  ...
  it('renders signpost drawings with measured boxes', () => {
    ...
    expect(ctx.calls).toContain('fillRect:32,40,36,20:rgba(245, 197, 66, 0.12):1');
    expect(ctx.calls).toContain('fillText:Post:38,50:#111:left:1:12px sans-serif');
  });
  ...
  it('renders price note drawings with a pointer and measured text box', () => {
    const ctx = new RecordingCanvasContext();
    const drawing: UserDrawing = {
      ...base,
      id: 'price-note',
