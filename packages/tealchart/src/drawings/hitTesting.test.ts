...
describe('user drawing hit testing', () => {
  ...
  it('hits image annotation edges and corner handles', () => {
    ...
    expect(hitTestUserDrawing(drawing, { x: 50, y: 10 }, space)?.drawing.id).toBe('image');
  ...
  it('hits fib spiral paths and endpoint handles', () => {
    ...
    expect(hitTestUserDrawing(drawing, { x: 50, y: 50 }, space)?.drawing.id).toBe('fib-spiral');
    expect(hitTestUserDrawing(drawing, { x: 50, y: 50 }, space)).toMatchObject({
      handle: 'end',
    });
  });
  ...
  it('hits gann box grids and corner handles', () => {
    const drawing: UserDrawing = {
      ...base,
      id: 'gann-box',
      kind: 'gannBox',
    ...
    expect(hitTestUserDrawing(drawing, { x: 50, y: 50 }, space)?.drawing.id).toBe('gann-box');
    expect(hitTestUserDrawing(drawing, { x: 90, y: 80 }, space)).toMatchObject({
      handle: 'bottomRight',
    });
  });

  it('hits gann square grids and corner handles', () => {
    const drawing: UserDrawing = {
      ...base,
      id: 'gann-square',
      kind: 'gannSquare',
    ...
    expect(hitTestUserDrawing(drawing, { x: 50, y: 50 }, space)?.drawing.id).toBe('gann-square');
    expect(hitTestUserDrawing(drawing, { x: 70, y: 80 }, space)).toMatchObject({
      handle: 'bottomRight',
    });
  });
  ...
  it('hits anchored volume profile bins, bounds, and anchor handle', () => {
    const drawing: UserDrawing = {
      ...base,
    ...
    expect(hitTestUserDrawing(drawing, { x: 50, y: 22 }, profileSpace)?.drawing.id).toBe(
      'anchored-volume-profile',
    );
    expect(hitTestUserDrawing(drawing, { x: 50, y: 45 }, profileSpace)?.drawing.id).toBe(
      'anchored-volume-profile',
    );
    expect(hitTestUserDrawing(drawing, { x: 95, y: 20 }, profileSpace)?.drawing.id).toBe(
      'anchored-volume-profile',
    );
    expect(hitTestUserDrawing(drawing, { x: 10, y: 25 }, profileSpace)).toMatchObject({
      handle: 'center',
    });
    expect(hitTestUserDrawing(drawing, { x: 95, y: 50 }, profileSpace, { tolerance: 1 })).toBeNull();
  });
  ...
  it('hits price label drawings using a configurable label box', () => {
    const drawing: UserDrawing = {
      ...base,
      id: 'price-label',
      kind: 'priceLabel',
  ...
  it('hits callout drawings by text box and pointer segment', () => {
    ...
    expect(hitTestUserDrawing(drawing, { x: 30, y: 70 }, space, { labelWidth: 50, labelHeight: 20 })?.drawing.id).toBe(
      'callout',
    );
    expect(hitTestUserDrawing(drawing, { x: 80, y: 50 }, space, { labelWidth: 50, labelHeight: 20 })).toBeNull();
  });

  it('hits price note drawings by text box and pointer segment', () => {
    const drawing: UserDrawing = {
      ...base,
      id: 'price-note',
      kind: 'priceNote',
  ...
  it('hits XABCD pattern segments and five anchor handles', () => {
    const drawing: UserDrawing = {
      ...base,
      id: 'xabcd',
      kind: 'xabcdPattern',
    ...
    expect(hitTestUserDrawing(drawing, { x: 20, y: 20 }, space)?.drawing.id).toBe('xabcd');
  ...
  it('hits three drives pattern segments and five anchor handles', () => {
    ...
    expect(hitTestUserDrawing(drawing, { x: 20, y: 20 }, space)?.drawing.id).toBe('three-drives');
    expect(hitTestUserDrawing(drawing, { x: 90, y: 10 }, space)).toMatchObject({
      drawing,
      handle: 'center',
      pointIndex: 4,
  ...
  it('hits Elliott triple combo wave paths and five anchor handles', () => {
    const drawing: UserDrawing = {
      ...base,
    ...
    expect(hitTestUserDrawing(drawing, { x: 20, y: 40 }, space)?.drawing.id).toBe('elliott-triple-combo');
    expect(hitTestUserDrawing(drawing, { x: 90, y: 55 }, space)).toMatchObject({
      drawing,
      handle: 'center',
      pointIndex: 4,
