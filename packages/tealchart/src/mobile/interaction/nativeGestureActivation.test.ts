...
describe('native gesture activation', () => {
  ...
  it('clears committed viewport gesture active flags so the next gesture can begin', () => {
    const panActive = shared(false);
    const pinchActive = shared(false);
    const priceScaleActive = shared(false);
    const timeScaleActive = shared(false);
    ...
    const panTransaction = gestureTransaction();
    const panGesture = createNativeChartPanGesture({
      beginNativeViewportInteraction: () => {},
      cancelNativeViewportInteraction: () => {},
      chartPanGestureState: {
    ...
    panGesture.handlers.onTouchesDown({ changedTouches: [{ x: 80, y: 60 }], allTouches: [{ x: 80, y: 60 }] }, mockStateManager());
    panGesture.handlers.onBegin({ x: 80, y: 60 });
    panGesture.handlers.onUpdate({ translationX: 8, translationY: 8 });
    panGesture.handlers.onEnd({ translationX: 8, translationY: 8 });
    expect(panActive.value).toBe(true);
    panGesture.handlers.onFinalize({}, false);
    expect(panActive.value).toBe(false);
    const secondPanStart = mockStateManager();
    panGesture.handlers.onTouchesDown({ changedTouches: [{ x: 80, y: 60 }], allTouches: [{ x: 80, y: 60 }] }, secondPanStart);
    expect(secondPanStart.failed).toBe(false);

    const priceScaleTransaction = gestureTransaction();
    const priceScaleGesture = createNativePriceScaleGesture({
      beginNativeViewportInteraction: () => {},
      cancelNativeViewportInteraction: () => {},
      commitPanViewport: () => {},
      ...
      priceScaleGestureState: {
        ...
        activeAnchorPrice: shared(0),
        indicatorPaneTarget: shared(null),
        transaction: priceScaleTransaction,
      },
      sharedViewport: viewport,
    }) as any;
    priceScaleGesture.handlers.onTouchesDown(
      { changedTouches: [{ x: frame.priceAxisHitLeft + 5, y: 80 }], allTouches: [{ x: frame.priceAxisHitLeft + 5, y: 80 }] },
      mockStateManager(),
    );
    priceScaleGesture.handlers.onBegin({ y: 80 });
    priceScaleGesture.handlers.onUpdate({ translationY: 8 });
    priceScaleGesture.handlers.onEnd();
    expect(priceScaleActive.value).toBe(true);
    priceScaleGesture.handlers.onFinalize({}, false);
    expect(priceScaleActive.value).toBe(false);
    const secondPriceScaleStart = mockStateManager();
    priceScaleGesture.handlers.onTouchesDown(
      { changedTouches: [{ x: frame.priceAxisHitLeft + 5, y: 80 }], allTouches: [{ x: frame.priceAxisHitLeft + 5, y: 80 }] },
      secondPriceScaleStart,
    );
    expect(secondPriceScaleStart.failed).toBe(false);

    const timeScaleTransaction = gestureTransaction();
    const timeScaleGesture = createNativeTimeScaleGesture({
      beginNativeViewportInteraction: () => {},
      cancelNativeViewportInteraction: () => {},
      commitPanViewport: () => {},
    ...
    timeScaleGesture.handlers.onTouchesDown(
      { changedTouches: [{ x: 80, y: frame.timeAxisTop + 5 }], allTouches: [{ x: 80, y: frame.timeAxisTop + 5 }] },
      mockStateManager(),
    );
    timeScaleGesture.handlers.onBegin();
    timeScaleGesture.handlers.onUpdate({ translationX: 8 });
    timeScaleGesture.handlers.onEnd();
    expect(timeScaleActive.value).toBe(true);
    timeScaleGesture.handlers.onFinalize({}, false);
    expect(timeScaleActive.value).toBe(false);
    const secondTimeScaleStart = mockStateManager();
    timeScaleGesture.handlers.onTouchesDown(
      { changedTouches: [{ x: 80, y: frame.timeAxisTop + 5 }], allTouches: [{ x: 80, y: frame.timeAxisTop + 5 }] },
      secondTimeScaleStart,
    );
    expect(secondTimeScaleStart.failed).toBe(false);
  });
