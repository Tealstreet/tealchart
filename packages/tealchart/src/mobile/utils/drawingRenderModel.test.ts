...
describe('mobile user drawing render model', () => {
  ...
  it('returns Skia-ready price range labels at the configured vertical position', () => {
    const positionedStyle = { ...style, measurementLabelPosition: 'top' as const };
    const state: UserDrawingState = {
      version: 1,
      activeTool: 'select',
      selection: null,
  ...
  it('returns Skia-ready date range labels at the configured vertical position', () => {
    const positionedStyle = { ...style, measurementLabelPosition: 'bottom' as const };
    const state: UserDrawingState = {
      version: 1,
      activeTool: 'select',
      selection: null,
    ...
    expect(resolveMobileUserDrawingRenderModel(state, new Map([[durationSpace.pane.id, durationSpace]]))[0]).toMatchObject({
      kind: 'dateRange',
      labelPoint: { x: 40, y: 88 },
      label: '3 bars, 1 minute',
      style: positionedStyle,
    });
  });

  it('returns Skia-ready date range labels at the configured horizontal alignment', () => {
    const alignedStyle = { ...style, measurementLabelAlignment: 'right' as const };
    const state: UserDrawingState = {
      version: 1,
      activeTool: 'select',
  ...
  it('returns Skia-ready date and price range labels at the configured vertical position', () => {
    const positionedStyle = { ...style, measurementLabelPosition: 'bottom' as const };
    const state: UserDrawingState = {
      version: 1,
      activeTool: 'select',
