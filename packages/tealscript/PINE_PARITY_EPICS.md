...
- Compatibility fixtures for real Pine idioms including MTF filters,
  ...
  drawing zones, multi-symbol screeners, imported library helpers, alerts,
  request-limit reuse, strategy entry/exit flows, default broker-emulator path
  and gap-fill behavior, and common drawing/table patterns.
...
6. Add a checkpoint coverage index by public-script idiom: MTF filters,
   ...
   session filters, exchange session-state gates, dashboard tables, drawing
   zones, screeners, library helpers, and UDT object-method state.
...
5. Call-binding diagnostics sweep: keep signed built-in namespace typo
   ...
   bindings, drawing constructor argument signatures, typed `input.*` default
   and literal constraint diagnostics, and `input.int()` / `input.float()`
   range-vs-options overload diagnostics, plus literal strategy order value
