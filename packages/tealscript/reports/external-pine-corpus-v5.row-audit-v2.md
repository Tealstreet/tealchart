...
# External Pine Corpus V5 Row Audit V2
...
Applying the 54 invalid-Pine verdicts in this audit to the raw v5 buckets changes the headline to `832 supported / 40 TealScript gap / 118 invalid Pine / 10 unsupported-by-design`. The comparable achievable denominator is `872`, so the audited support rate is `832/872 = 95.41%`. The three remaining rows in this audit classified as real gaps are not removed from that denominator.
