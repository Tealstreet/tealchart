...
- Vector cases: 480; passing 477; expected-red 3.
- Official members covered by at least one vector case: 811/861 (94.19%).
- Unmapped official members: 50.
- Unmapped but locally verifiable without TradingView traces: 0.
- Unmapped trace/host-required: 50.
- Practical no-trace coverage: 811/811 (100.00%).
...
- `strategy.risk.max_drawdown` - exact strategy risk halt and forced-exit timing needs TradingView broker-emulator/session traces
...
- `strategy.risk.max_intraday_loss` - exact strategy risk halt and forced-exit timing needs TradingView broker-emulator/session traces
