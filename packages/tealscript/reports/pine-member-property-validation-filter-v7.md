# Pine Member Property Validation Filter V7

> Superseded by `pine-member-property-validation-filter-v8.md`. Historical measurement only.
...
- Scalar/proxy/tuple/collection rules unlocked by constructed single-target cases: 12.
- Scalar/proxy/tuple/collection rules still stranded behind judgment cases: 143.
- Combined known-good validation coverage after shipped studies: 714/993 (71.90%).
...
- single-target passing cases: 233
- multi-target passing cases: 130
- zero-target passing cases: 341
...
- Constructed single-target sample unlocked 12 rules.
...
- Completion estimate: 143 scalar/proxy/tuple/collection rules still remain stranded after subtracting rules already validated by single-target cases; at the sample rate, finishing all constructible cases is a several-hour task and not an automatic sweep.
