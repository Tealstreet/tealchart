Superseded by `pine-rule6-backward-audit-v6.md`. Historical measurement only.

# Pine Rule 6 Backward Audit V5

Measurement commit: `d2e4d8a545`.
...
| --- | --- | --- |
| `language.root-if-local-shadows-builtin-history` | open expected-red, runtime-owned | Resolver/history fix confirmed by the existing value vector derived from documented conditional local-block, variable-scope and history semantics. Current output is all `na`; expected is previous `close + 10`. |
| `language.nested-if-local-shadows-builtin-history` | open expected-red, runtime-owned | Resolver/history fix confirmed by the existing value vector derived from documented nested conditional local-block, variable-scope and history semantics. Current output is all `na`; expected is previous `close + 20`. |
...
| --- | --- | --- | --- |
| Root if-expression local declaration with history | Conditional structures, variable declarations and operators docs: branch-local `n` shadows any built-in fallback and `n[1]` reads local series history. | `language.root-if-local-shadows-builtin-history` in `pine-value-vectors-coverage-v165.json`. | open: current output all `na` |
| Nested if-expression local declaration with history | Nested conditional local blocks return final expressions; local declaration scope controls `n[1]`. | `language.nested-if-local-shadows-builtin-history` in `pine-value-vectors-coverage-v165.json`. | open: current output all `na` |
...
## Shared-Path Fixes Retained From V4
