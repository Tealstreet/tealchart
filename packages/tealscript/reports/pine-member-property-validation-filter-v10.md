# Pine Member Property Validation Filter V10

> Superseded by `pine-member-property-validation-filter-v11.md`. Historical measurement only.
...
- Scalar/proxy/tuple/collection rules unlocked by constructed single-target cases: 98.
- Scalar/proxy/tuple/collection rules still stranded behind judgment cases: 57.
- Combined known-good validation coverage after shipped studies: 800/993 (80.56%).
...
- Constructed single-target sample unlocked 98 rules.
...
- Completion estimate: 57 scalar/proxy/tuple/collection rules still remain stranded after subtracting rules already validated by single-target cases; at the sample rate, finishing all constructible cases is a several-hour task and not an automatic sweep.
