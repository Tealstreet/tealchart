# Pine Value Vector Assertion Quality V8

Superseded by `pine-value-vector-assertion-quality-v9.md`. Historical measurement only.

## Basis

- Source: `packages/tealscript/scripts/run-pine-value-vectors.ts` at current HEAD.
- Member denominator: the 811 official names already covered by at least one vector case in `pine-value-vector-member-map-v29`.
