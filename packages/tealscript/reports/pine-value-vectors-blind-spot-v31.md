...
# Pine Value Vectors Blind Spot V31
...
- `/tmp/pine-value-vectors-currency-v1.json`
- `pine-value-vectors-practical-ceiling-v1.md`
- `pine-value-vectors-blind-spot-v30.md`

## Summary

- Independent-oracle value-vector cases: `361`.
...
| --- | ---: | ---: | --- |
| Raw builtin audit denominator | `431/489` | `88.14%` | No net numerator movement claimed; this batch makes the already-accounted currency constant surface explicit. |
...
## New Currency Coverage

- Added one explicit vector covering all `56` documented `currency.*` constants: `AED`, `ARS`, `AUD`, `BDT`, `BHD`, `BRL`, `BTC`, `CAD`, `CHF`, `CLP`, `CNY`, `COP`, `CZK`, `DKK`, `EGP`, `ETH`, `EUR`, `GBP`, `HKD`, `HUF`, `IDR`, `ILS`, `INR`, `ISK`, `JPY`, `KES`, `KRW`, `KWD`, `LKR`, `MAD`, `MXN`, `MYR`, `NGN`, `NOK`, `NONE`, `NZD`, `PEN`, `PHP`, `PKR`, `PLN`, `QAR`, `RON`, `RSD`, `RUB`, `SAR`, `SEK`, `SGD`, `THB`, `TND`, `TRY`, `TWD`, `USD`, `USDT`, `VES`, `VND`, and `ZAR`.
- Each constant is asserted as its exact string code through a plotted equality check.

## Practical Ceiling

- Remaining raw uncovered members: `58`.
- Remaining no-trace-verifiable members: `40`.
- The remaining no-trace-verifiable set is still mostly the strategy aggregate/max-contract defect, strategy risk edge cases, low-use visual residues, and the historical/realtime language boundary around `varip`.

## Gate Status

- Value-vector gate: `361` cases, `357` compiled/public matches, `4` expected failures, `0` unexpected failures, `0` unexpected passes.
