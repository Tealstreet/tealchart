...
# External Pine Corpus v5 Fixture Profile Delta v3

## Basis

- Measurement commit: `70cf82be24f441578dc66dca9e5aa9e137de489a`.
...
- Daily report: `/Users/samuelsteady/cs/tealstreet-next/.aimux/worktrees/tealscript-parity/packages/tealscript/.cache/tealscript/pine-corpus-v5-20260910/fixture-profiles/014ff37f39-v3/external-pine-corpus-v5-daily-standard.json`.
- Context-stress report: `/Users/samuelsteady/cs/tealstreet-next/.aimux/worktrees/tealscript-parity/packages/tealscript/.cache/tealscript/pine-corpus-v5-20260910/fixture-profiles/014ff37f39-v3/external-pine-corpus-v5-context-stress.json`.
...
- Daily profile: 855/1000 raw output (85.5%); 855/926 achievable output (92.33%); validity {"corpus-hygiene":8,"invalid-pine":56,"supported":860,"tealscript-gap":66,"unsupported-by-design":10}.
- Context-stress profile: 855/1000 raw output (85.5%); 855/926 achievable output (92.33%); validity {"corpus-hygiene":8,"invalid-pine":56,"supported":860,"tealscript-gap":66,"unsupported-by-design":10}.
...
| Row | Daily output | Context-stress output | Result |
| --- | ---: | ---: | --- |
| `sources/0511__casoon-pine-scripts__market_scenario_projector.pine` | 13 drawings | 13 drawings | Recovered. The stress profile now ends with a volatile daily-compatible final window that clears the script's `barstate.islast` scenario gate. |
| `sources/0759__iamhuraira-trading-view-script__FVG_with_IFVG_Indicator.pine` | 1 drawing | 1 drawing | Recovered. The final window now carries the same kind of recent gap/inversion shape that the daily fixture already reached. |
| `sources/0769__TraderOracle-TradingView__StarFragment.pine` | 500 drawings | 500 drawings | Recovered. The stress profile includes the `08:45` session minute required by `time(timeframe.period, "0845-0846")`. |
...
The context-stress profile no longer trails the daily profile because of final-window or session misses: both profiles now produce `855/926` achievable output with zero supported-output regressions between them. The remaining `138` changed rows are chart-context-sensitive payload changes, not support losses. The remaining trace-only list is narrower: realtime tick behavior, real symbol metadata, event-backed request feeds, and exact exchange calendars.
