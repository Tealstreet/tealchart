# Pine Member Property Validation Filter V11

> Superseded by `pine-member-property-validation-filter-v12.md`. Historical measurement only.
...
- Scalar/proxy/tuple/collection rules unlocked by constructed single-target cases: 124.
- Scalar/proxy/tuple/collection rules still stranded behind judgment cases: 31.
- Combined known-good validation coverage after shipped studies: 826/993 (83.18%).
...
- Constructed single-target sample unlocked 124 rules.
...
- Completion estimate: 31 scalar/proxy/tuple/collection rules still remain stranded after subtracting rules already validated by single-target cases; at the sample rate, finishing all constructible cases is a several-hour task and not an automatic sweep.
