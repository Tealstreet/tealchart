# Pine Value Vector Live Reference Denominator V3

Measurement commit: `d9d1620246`.
...
| --- | ---: | ---: | ---: | --- |
| Committed snapshot | `814` | `861` | `94.54%` | Coverage against the checked-in artifact used by existing member maps. |
| Snapshot plus live-reference callable correction | `814` | `886` | `91.87%` | Current total-member answer to how much live-documented Pine member surface has value-vector coverage. |
| Practical no-trace denominator | `814` | `814` | `100.00%` | Excludes the trace/host/declaration/error-bound names that cannot receive deterministic local value vectors. |
...
`pine-value-vector-member-map-v31.json` stores `traceOrHostRequired` entries as
...
Use `814/886` (`91.87%`) for total live-reference member coverage. Use
`814/814` (`100.00%`) for practical deterministic no-trace value coverage. Do
