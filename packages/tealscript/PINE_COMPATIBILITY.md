...
window helpers such as `math.sum()` and common `ta.*` source-window helpers
report their observed prior-bar lookback requirements as well. Truly dynamic
series offsets are still inferred from observed runtime access.
...
covered rolling-window helper lookbacks such as `math.sum()` and common `ta.*`
source-window helpers. Full Pine-style preallocation for arbitrary series
offsets remains a compatibility target. The checkpoint corpus tracks an
official `max_bars_back`
