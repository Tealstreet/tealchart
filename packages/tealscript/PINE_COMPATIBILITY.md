...
- Tuple destructuring preserves known positional types from literal tuple
  ...
  user-function loop and switch tuple returns, direct user-method tuple returns,
  and supported tuple-returning TA calls, so downstream assignment diagnostics
  can use destructured names.
