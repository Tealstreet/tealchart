...
for cases that intentionally stop before datafeed or render comparison. Corpus
reports also separate planned unsupported blockers from actionable failures so
known host/data-model deferrals stay visible without looking like regressions.
...
filters, public Parabolic SAR reversal routing, public linear-regression and
Keltner channels, public moving-average ribbons, public stochastic, MFI, CCI,
CMO, TSI, and Williams %R oscillator signals, public ROC and Momentum signals, public zigzag polylines, public
...
calls, including arithmetic source expressions, arithmetic ternary/switch
branches, multiline block `if` returns, multiline block `switch` returns, and
block-local `if` initializers.
...
Parabolic SAR reversal, linear-regression channel, stochastic and MFI
