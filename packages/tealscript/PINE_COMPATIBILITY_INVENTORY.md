...
| Parser/runtime | Variable declarations | Supported | `tests/parser/parser.test.ts`; `tests/runtime/runtime.test.ts`; `src/semantic/checker.test.ts` | Annotated local primitive, reference, collection, UDT, enum, and mixed conditional/switch initializer mismatches are diagnosed. Broader qualifier-sensitive type checking is planned. |
