...
| Built-ins | `ta.*` | Partial | `tests/compat/pine-basics.test.ts`; `tests/compat/pine-builtins.test.ts`; `src/runtime/engine.test.ts`; `src/semantic/checker.test.ts` | Runtime covers common moving-average, oscillator, statistical, event, cross, pivot, DMI, SAR, and linear-regression helpers. Semantic analysis preserves tuple returns plus known scalar bool/int/float TA helper return types, including source-preserving `ta.change()` and `ta.valuewhen()` results. Full reference-manual inventory and edge-case parity remain. |
...
| Built-ins | `color.*` | Partial | `tests/compat/pine-builtins.test.ts`; `tests/compat/pine-visuals.test.ts`; `src/semantic/checker.test.ts` | Runtime covers common constructors, channels, and gradients. Semantic analysis preserves known color constructor and channel return types for downstream diagnostics. Exact named constants and theme-sensitive behavior remain. |
| Built-ins | `str.*` | Partial | `tests/compat/pine-builtins.test.ts`; `src/semantic/checker.test.ts` | Runtime covers common format, search, split, case, trim, replace, and repeat helpers. Semantic analysis preserves known string helper return types for downstream diagnostics, including `str.split()` array element types. Formatting and Unicode edge cases remain. |
