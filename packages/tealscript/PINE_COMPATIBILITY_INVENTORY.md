...
| Parser/runtime | Tuple destructuring | Supported | `tests/compat/pine-basics.test.ts`; `src/semantic/checker.test.ts` | Literal tuple destructuring and known tuple-returning TA calls infer positional element types for downstream diagnostics. Broader tuple-return inference remains planned. |
