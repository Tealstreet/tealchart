...
| Parser/runtime | Wrapped delimited syntax | Partial | `tests/parser/parser.test.ts`; `tests/compat/pine-language.test.ts` | Multiline declarations, calls, arrays, tuple patterns, index access, and parenthesized expressions are supported. Arbitrary operator line-continuation and deeper indentation edge cases remain. |
