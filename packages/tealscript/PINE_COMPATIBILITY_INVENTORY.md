...
| Parser/runtime | Tuple destructuring | Supported | `tests/compat/pine-basics.test.ts`; `src/semantic/checker.test.ts` | Literal tuple destructuring, direct user-function tuple returns, and known tuple-returning TA calls infer positional element types for downstream diagnostics. Broader branch and loop tuple-return inference remains planned. |
