...
| Built-ins | `array.*` | Partial | `src/runtime/arrays.test.ts`; `tests/compat/pine-arrays.test.ts`; `tests/compat/pine-builtins.test.ts`; `src/semantic/checker.test.ts` | Runtime covers common constructors including `array.new<T>`, mutation, search, ordering, slicing, and statistical helpers. Semantic coverage validates `array.new<T>` template arity and qualifier misuse. Full array reference inventory and edge cases remain. |
