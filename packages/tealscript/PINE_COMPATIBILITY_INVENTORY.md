...
| Parser/runtime | Tuple destructuring | Supported | `tests/compat/pine-basics.test.ts`; `src/semantic/checker.test.ts` | Literal tuple destructuring, direct and compatible if/else plus partial-if user-function tuple returns, direct user-function loop and defaulted or partial switch tuple returns, compatible user-method tuple returns, and known tuple-returning TA calls infer positional element types for downstream diagnostics. Broader mixed-shape branch, loop, and switch tuple-return inference remains planned. |
...
| Parser/runtime | `if` / `else if` / `else` | Supported | `tests/parser/parser.test.ts`; `tests/compat/pine-language.test.ts`; `src/semantic/checker.test.ts` | Compatible user-function if/else and partial if return arms infer scalar types for downstream diagnostics. Nested indentation hardening continues under core semantics. |
