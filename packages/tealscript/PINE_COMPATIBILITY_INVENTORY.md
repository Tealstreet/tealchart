...
| Parser/runtime | Variable declarations | Supported | `tests/parser/parser.test.ts`; `tests/runtime/runtime.test.ts`; `src/semantic/checker.test.ts` | Annotated local UDT and enum variable mismatches are diagnosed. Broader primitive, collection, and qualifier-sensitive type checking is planned. |
