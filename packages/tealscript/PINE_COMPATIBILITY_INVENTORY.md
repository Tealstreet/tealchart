...
| Built-ins | `array.*` | Partial | `src/runtime/arrays.test.ts`; `tests/compat/pine-arrays.test.ts`; `tests/compat/pine-builtins.test.ts`; `src/semantic/checker.test.ts` | Runtime covers common constructors including `array.new<T>`, mutation, search, ordering, slicing, and statistical helpers. Semantic coverage preserves element types from `array.new<T>` and typed constructors, validates generic constructor template arity and qualifier misuse, and reports conservative mutation value mismatches for known arrays. Full array reference inventory and edge cases remain. |
