...
| Parser/runtime | User-defined functions | Partial | `tests/parser/parser.test.ts`; `tests/compat/pine-language.test.ts`; `src/semantic/checker.test.ts` | Single-line and multiline UDFs, named/default arguments, invalid call-shape diagnostics, annotated argument type/qualifier diagnostics, branch and loop returns, nested helper return inference, call-site qualifier propagation for inferred returns, and recursive-call diagnostics are covered. Deeper layout parser hardening for arbitrary nested pasted blocks and the full Pine qualified type/runtime contract remain partial. |
| Parser/runtime | Methods, e.g. `arr.push(x)` and `method scale(float this)` | Partial | `tests/compat/pine-arrays.test.ts`; `tests/compat/pine-objects.test.ts`; `src/runtime/arrays.test.ts`; `src/semantic/checker.test.ts` | Built-in collection method sugar and user-defined method declarations dispatch. Local and imported UDT receiver overloads are covered at runtime; local receiver mismatch diagnostics, method return inference, method overload declarations, and method argument shape/type/qualifier diagnostics are covered semantically. Enum/imported method semantic edge cases and complete overload selection by full parameter signatures remain planned. |
| Parser/runtime | `switch` | Partial | `tests/parser/parser.test.ts`; `tests/compat/pine-control-time.test.ts`; `src/semantic/checker.test.ts` | Expression switches, multiline branch bodies, and semantic branch return inference for final `if` blocks and nested helper calls are covered. Exhaustive type diagnostics remain planned. |
...
| Runtime | Function-local series state | Partial | `tests/compat/pine-language.test.ts`; `src/semantic/checker.test.ts` | Function-local `var`/`varip` persistence and semantic call-site qualifier inference for UDF/method returns are covered. Runtime nested-block execution/state edge cases and the full Pine qualified type/runtime contract remain partial. |
...
| Built-ins | Global helpers | Partial | `src/runtime/engine.test.ts`; `tests/compat/pine-language.test.ts`; `src/semantic/checker.test.ts` | `na`, `nz`, `fixnan`, and primitive casts are covered for common numeric/source idioms, with semantic return inference for helper calls. Broader type-system diagnostics and exact color overload parity are planned. |
