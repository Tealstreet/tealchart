...
| Drawings | `table.*` | Partial | `src/runtime/drawings/builtins.test.ts`; `packages/tealchart/src/rendering/TealScriptDrawingRenderer.test.ts` | Runtime covers fixed-position handles, delete/clear, cells, text/color/background/size setters, width/height setters, alignment setters, and a 10,000-cell sandbox cap. Renderer covers fixed table placement, measured cell sizes, backgrounds, borders, frames, and text alignment. Remaining gaps are full Pine table setter surface and TradingView-exact sizing. |
