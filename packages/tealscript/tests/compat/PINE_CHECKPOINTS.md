...
| Request data and multi-timeframe requests | `Public MTF Trend Checkpoint`; `Public Currency Conversion Checkpoint`; `Official Lower TF Array Checkpoint`; repaint-safe HTF fixture in `pine-request-security.test.ts` | `pine-real-checkpoints.test.ts`; `pine-request-security.test.ts` |
