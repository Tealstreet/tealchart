...
| Request data and multi-timeframe requests | `Public MTF Trend Checkpoint`; `Public Currency Conversion Checkpoint`; `Public Earnings Event Checkpoint`; `Public Financial Dashboard Checkpoint`; `Public Economic Macro Checkpoint`; `Official Lower TF Array Checkpoint`; repaint-safe HTF fixture in `pine-request-security.test.ts` | `pine-real-checkpoints.test.ts`; `pine-request-security.test.ts` |
