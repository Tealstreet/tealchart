...
| Marker output payloads | `Official Marker Payload Checkpoint`; `Public Marker Signal Checkpoint`; `Public Earnings Event Checkpoint`; marker color/text payload fixtures in `pine-visuals.test.ts` | `pine-real-checkpoints.test.ts`; `pine-visuals.test.ts` |
...
| Request data and multi-timeframe requests | `Public MTF Trend Checkpoint`; `Public Currency Conversion Checkpoint`; `Public Earnings Event Checkpoint`; `Official Lower TF Array Checkpoint`; repaint-safe HTF fixture in `pine-request-security.test.ts` | `pine-real-checkpoints.test.ts`; `pine-request-security.test.ts` |
...
| Drawings and tables | `Public Drawing Zone Checkpoint`; `Public Linefill Channel Checkpoint`; `Public Zigzag Polyline Checkpoint`; manual comparison milestones plus reduced drawing fixtures | `pine-real-checkpoints.test.ts`; `PINE_CHECKPOINTS.md`; `pine-drawings.test.ts` |
