...
| `Official Max Drawdown Risk Checkpoint` | https://www.tradingview.com/pine-script-docs/concepts/strategies/ | `strategy.risk.max_drawdown(value=5, type=strategy.cash)` blocks additional non-exit submissions after overall equity drawdown exceeds the cash limit. | Position, equity plot, fill ids, submitted order ledger, equity-curve drawdown, and captured risk-rule metadata over three local bars. |
