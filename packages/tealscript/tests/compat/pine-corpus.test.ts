...
describe('Pine compatibility checkpoint corpus', () => {
  it('keeps source-linked reduced checkpoints in the offline corpus', () => {
    ...
    expect(compatibilityCheckpointLedger.entries).toHaveLength(20);
    expect(run.summary.total).toBe(20);
    expect(run.summary.passed).toBe(20);
    ...
    expect(run.summary.byFeatureTag).toMatchObject({
      ...
      strategy: { total: 5, passed: 5, failed: 0 },
  ...
  it('renders a stable checkpoint corpus report', () => {
    ...
    expect(markdown).toContain('Total: 20');
    ...
    expect(markdown).toContain('| strategy | 5 | 5 | 0 |');
  ...
  it('builds a checkpoint coverage index from intake metadata', () => {
    ...
    expect(index).toMatchObject({
      schemaVersion: 1,
      total: 20,
      byCategory: { indicator: 15, strategy: 5 },
      bySourceKind: { official_docs: 17, public_script: 3 },
      byPineVersion: { v6: 20 },
      byStoragePolicy: { reduced_fixture_only: 20 },
    });
    ...
    expect(markdown).toContain('Total checkpoints: 20');
    expect(markdown).toContain('| official_docs | 17 |');
    expect(markdown).toContain('| reduced_fixture_only | 20 |');
  });
});
