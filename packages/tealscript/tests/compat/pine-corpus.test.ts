...
describe('Pine compatibility checkpoint corpus', () => {
  it('keeps source-linked reduced checkpoints in the offline corpus', () => {
    ...
    expect(compatibilityCheckpointLedger.entries).toHaveLength(16);
    expect(run.summary.total).toBe(16);
    expect(run.summary.passed).toBe(16);
  ...
  it('renders a stable checkpoint corpus report', () => {
    ...
    expect(markdown).toContain('Total: 16');
  ...
  it('builds a checkpoint coverage index from intake metadata', () => {
    ...
    expect(index).toMatchObject({
      schemaVersion: 1,
      total: 16,
      byCategory: { indicator: 12, strategy: 4 },
      bySourceKind: { official_docs: 13, public_script: 3 },
      byPineVersion: { v6: 16 },
      byStoragePolicy: { reduced_fixture_only: 16 },
    });
    ...
    expect(markdown).toContain('Total checkpoints: 16');
    expect(markdown).toContain('| official_docs | 13 |');
    expect(markdown).toContain('| reduced_fixture_only | 16 |');
  });
});
