...
describe('Pine compatibility checkpoint corpus', () => {
  it('keeps source-linked reduced checkpoints in the offline corpus', () => {
    ...
    expect(compatibilityCheckpointLedger.entries).toHaveLength(92);
    expect(run.summary.total).toBe(92);
    expect(run.summary.passed).toBe(91);
    ...
    expect(run.summary.byFeatureTag).toMatchObject({
      ...
      trend_filter: { total: 5, passed: 5, failed: 0 },
      ...
      channels: { total: 2, passed: 2, failed: 0 },
      ...
      oscillator: { total: 3, passed: 3, failed: 0 },
      ...
      signals: { total: 34, passed: 34, failed: 0 },
      declaration_metadata: { total: 1, passed: 1, failed: 0 },
      output: { total: 23, passed: 23, failed: 0 },
      supertrend: { total: 1, passed: 1, failed: 0 },
      ta: { total: 11, passed: 11, failed: 0 },
  ...
  it('renders a stable checkpoint corpus report', () => {
    ...
    expect(markdown).toContain('Total: 92');
    expect(markdown).toContain('Passed: 91');
    expect(markdown).toContain('Failed: 1');
    expect(markdown).toContain('Planned unsupported: 1');
    expect(markdown).toContain('Actionable failed: 0');
    expect(markdown).toContain('Pass rate: 98.9%');
    ...
    expect(markdown).toContain('| channels | 2 | 2 | 0 |');
    ...
    expect(markdown).toContain('| oscillator | 3 | 3 | 0 |');
    ...
    expect(markdown).toContain('| signals | 34 | 34 | 0 |');
    expect(markdown).toContain('| declaration_metadata | 1 | 1 | 0 |');
    expect(markdown).toContain('| output | 23 | 23 | 0 |');
    expect(markdown).toContain('| supertrend | 1 | 1 | 0 |');
    expect(markdown).toContain('| ta | 11 | 11 | 0 |');
    ...
    expect(markdown).toContain('| trend_filter | 5 | 5 | 0 |');
...
`, {
  ...
  it('builds a checkpoint coverage index from intake metadata', () => {
    ...
    expect(index).toMatchObject({
      schemaVersion: 1,
      total: 92,
      byCategory: { indicator: 70, strategy: 22 },
      bySourceKind: { official_docs: 28, public_script: 64 },
      byPineVersion: { v4: 1, v6: 91 },
      byStoragePolicy: { reduced_fixture_only: 92 },
    });
    ...
    expect(markdown).toContain('Total checkpoints: 92');
    expect(markdown).toContain('| official_docs | 28 |');
    expect(markdown).toContain('| public_script | 64 |');
    expect(markdown).toContain('| reduced_fixture_only | 92 |');
    expect(formatPineCompatibilityCoverageJson(index)).toContain('"total": 92');
  });
  ...
  it('generates deterministic dashboard artifacts for CI', () => {
    ...
    try {
      ...
      expect(readFileSync(join(outDir, 'pine-compatibility-corpus.json'), 'utf8')).toContain('"passed": 91');
      ...
      expect(readFileSync(join(outDir, 'pine-compatibility-coverage.json'), 'utf8')).toContain('"total": 92');
      expect(readFileSync(join(outDir, 'pine-compatibility-coverage.md'), 'utf8')).toContain('Total checkpoints: 92');
    } finally {
