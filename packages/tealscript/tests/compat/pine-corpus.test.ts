...
describe('Pine compatibility checkpoint corpus', () => {
  it('keeps source-linked reduced checkpoints in the offline corpus', () => {
    ...
    expect(compatibilityCheckpointLedger.entries).toHaveLength(93);
    expect(run.summary.total).toBe(93);
    expect(run.summary.passed).toBe(92);
    ...
    expect(run.summary.byFeatureTag).toMatchObject({
      ...
      oscillator: { total: 4, passed: 4, failed: 0 },
      ...
      signals: { total: 35, passed: 35, failed: 0 },
      declaration_metadata: { total: 1, passed: 1, failed: 0 },
      output: { total: 24, passed: 24, failed: 0 },
      supertrend: { total: 1, passed: 1, failed: 0 },
      ta: { total: 12, passed: 12, failed: 0 },
  ...
  it('renders a stable checkpoint corpus report', () => {
    ...
    expect(markdown).toContain('Total: 93');
    expect(markdown).toContain('Passed: 92');
    ...
    expect(markdown).toContain('| oscillator | 4 | 4 | 0 |');
    ...
    expect(markdown).toContain('| signals | 35 | 35 | 0 |');
    expect(markdown).toContain('| declaration_metadata | 1 | 1 | 0 |');
    expect(markdown).toContain('| output | 24 | 24 | 0 |');
    expect(markdown).toContain('| supertrend | 1 | 1 | 0 |');
    expect(markdown).toContain('| ta | 12 | 12 | 0 |');
...
`, {
  ...
  it('builds a checkpoint coverage index from intake metadata', () => {
    ...
    expect(index).toMatchObject({
      schemaVersion: 1,
      total: 93,
      byCategory: { indicator: 71, strategy: 22 },
      bySourceKind: { official_docs: 28, public_script: 65 },
      byPineVersion: { v4: 1, v6: 92 },
      byStoragePolicy: { reduced_fixture_only: 93 },
    });
    ...
    expect(markdown).toContain('Total checkpoints: 93');
    expect(markdown).toContain('| official_docs | 28 |');
    expect(markdown).toContain('| public_script | 65 |');
    expect(markdown).toContain('| reduced_fixture_only | 93 |');
    expect(formatPineCompatibilityCoverageJson(index)).toContain('"total": 93');
  });
  ...
  it('generates deterministic dashboard artifacts for CI', () => {
    ...
    try {
      ...
      expect(readFileSync(join(outDir, 'pine-compatibility-corpus.json'), 'utf8')).toContain('"passed": 92');
      ...
      expect(readFileSync(join(outDir, 'pine-compatibility-coverage.json'), 'utf8')).toContain('"total": 93');
      expect(readFileSync(join(outDir, 'pine-compatibility-coverage.md'), 'utf8')).toContain('Total checkpoints: 93');
    } finally {
