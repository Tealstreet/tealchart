...
describe('Pine compatibility checkpoint corpus', () => {
  it('keeps source-linked reduced checkpoints in the offline corpus', () => {
    ...
    expect(compatibilityCheckpointLedger.entries).toHaveLength(21);
    expect(run.summary.total).toBe(21);
    expect(run.summary.passed).toBe(21);
    ...
    expect(run.summary.byFeatureTag).toMatchObject({
      ...
      strategy: { total: 6, passed: 6, failed: 0 },
  ...
  it('renders a stable checkpoint corpus report', () => {
    ...
    expect(markdown).toContain('Total: 21');
    ...
    expect(markdown).toContain('| strategy | 6 | 6 | 0 |');
  ...
  it('builds a checkpoint coverage index from intake metadata', () => {
    ...
    expect(index).toMatchObject({
      schemaVersion: 1,
      total: 21,
      byCategory: { indicator: 15, strategy: 6 },
      bySourceKind: { official_docs: 18, public_script: 3 },
      byPineVersion: { v6: 21 },
      byStoragePolicy: { reduced_fixture_only: 21 },
    });
    ...
    expect(markdown).toContain('Total checkpoints: 21');
    expect(markdown).toContain('| official_docs | 18 |');
    expect(markdown).toContain('| reduced_fixture_only | 21 |');
  });
});
