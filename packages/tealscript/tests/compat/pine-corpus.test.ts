...
describe('Pine compatibility checkpoint corpus', () => {
  it('keeps source-linked reduced checkpoints in the offline corpus', () => {
    ...
    expect(compatibilityCheckpointLedger.entries).toHaveLength(45);
    expect(run.summary.total).toBe(45);
    expect(run.summary.passed).toBe(45);
    ...
    expect(run.summary.byFeatureTag).toMatchObject({
      ...
      request: { total: 11, passed: 11, failed: 0 },
      ...
      visuals: { total: 12, passed: 12, failed: 0 },
      ...
      markers: { total: 2, passed: 2, failed: 0 },
      signals: { total: 6, passed: 6, failed: 0 },
  ...
  it('renders a stable checkpoint corpus report', () => {
    ...
    expect(markdown).toContain('Total: 45');
    ...
    expect(markdown).toContain('| request | 11 | 11 | 0 |');
    ...
    expect(markdown).toContain('| markers | 2 | 2 | 0 |');
    expect(markdown).toContain('| signals | 6 | 6 | 0 |');
  ...
  it('builds a checkpoint coverage index from intake metadata', () => {
    ...
    expect(index).toMatchObject({
      schemaVersion: 1,
      total: 45,
      byCategory: { indicator: 32, strategy: 13 },
      bySourceKind: { official_docs: 22, public_script: 23 },
      byPineVersion: { v6: 45 },
      byStoragePolicy: { reduced_fixture_only: 45 },
    });
    ...
    expect(markdown).toContain('Total checkpoints: 45');
    expect(markdown).toContain('| official_docs | 22 |');
    expect(markdown).toContain('| public_script | 23 |');
    expect(markdown).toContain('| reduced_fixture_only | 45 |');
    expect(formatPineCompatibilityCoverageJson(index)).toContain('"total": 45');
  });
  ...
  it('generates deterministic dashboard artifacts for CI', () => {
    ...
    try {
      ...
      expect(readFileSync(join(outDir, 'pine-compatibility-corpus.json'), 'utf8')).toContain('"passed": 45');
      expect(readFileSync(join(outDir, 'pine-compatibility-corpus.md'), 'utf8')).toContain('Pass rate: 100.0%');
      expect(readFileSync(join(outDir, 'pine-compatibility-coverage.json'), 'utf8')).toContain('"total": 45');
      expect(readFileSync(join(outDir, 'pine-compatibility-coverage.md'), 'utf8')).toContain('Total checkpoints: 45');
    } finally {
