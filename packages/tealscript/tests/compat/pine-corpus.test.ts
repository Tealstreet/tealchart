...
describe('Pine compatibility checkpoint corpus', () => {
  it('keeps source-linked reduced checkpoints in the offline corpus', () => {
    ...
    expect(compatibilityCheckpointLedger.entries).toHaveLength(28);
    expect(run.summary.total).toBe(28);
    expect(run.summary.passed).toBe(28);
    ...
    expect(run.summary.byFeatureTag).toMatchObject({
      ...
      signals: { total: 3, passed: 3, failed: 0 },
  ...
  it('renders a stable checkpoint corpus report', () => {
    ...
    expect(markdown).toContain('Total: 28');
    ...
    expect(markdown).toContain('| signals | 3 | 3 | 0 |');
  ...
  it('builds a checkpoint coverage index from intake metadata', () => {
    ...
    expect(index).toMatchObject({
      schemaVersion: 1,
      total: 28,
      byCategory: { indicator: 18, strategy: 10 },
      bySourceKind: { official_docs: 22, public_script: 6 },
      byPineVersion: { v6: 28 },
      byStoragePolicy: { reduced_fixture_only: 28 },
    });
    ...
    expect(markdown).toContain('Total checkpoints: 28');
    expect(markdown).toContain('| official_docs | 22 |');
    expect(markdown).toContain('| public_script | 6 |');
    expect(markdown).toContain('| reduced_fixture_only | 28 |');
    expect(formatPineCompatibilityCoverageJson(index)).toContain('"total": 28');
  });
  ...
  it('generates deterministic dashboard artifacts for CI', () => {
    ...
    try {
      ...
      expect(readFileSync(join(outDir, 'pine-compatibility-corpus.json'), 'utf8')).toContain('"passed": 28');
      expect(readFileSync(join(outDir, 'pine-compatibility-corpus.md'), 'utf8')).toContain('Pass rate: 100.0%');
      expect(readFileSync(join(outDir, 'pine-compatibility-coverage.json'), 'utf8')).toContain('"total": 28');
      expect(readFileSync(join(outDir, 'pine-compatibility-coverage.md'), 'utf8')).toContain('Total checkpoints: 28');
    } finally {
