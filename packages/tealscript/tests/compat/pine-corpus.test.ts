...
describe('Pine compatibility checkpoint corpus', () => {
  it('keeps source-linked reduced checkpoints in the offline corpus', () => {
    ...
    expect(compatibilityCheckpointLedger.entries).toHaveLength(46);
    expect(run.summary.total).toBe(46);
    expect(run.summary.passed).toBe(46);
    ...
    expect(run.summary.byFeatureTag).toMatchObject({
      ...
      visuals: { total: 13, passed: 13, failed: 0 },
  ...
  it('renders a stable checkpoint corpus report', () => {
    ...
    expect(markdown).toContain('Total: 46');
  ...
  it('builds a checkpoint coverage index from intake metadata', () => {
    ...
    expect(index).toMatchObject({
      schemaVersion: 1,
      total: 46,
      byCategory: { indicator: 33, strategy: 13 },
      bySourceKind: { official_docs: 22, public_script: 24 },
      byPineVersion: { v6: 46 },
      byStoragePolicy: { reduced_fixture_only: 46 },
    });
    ...
    expect(markdown).toContain('Total checkpoints: 46');
    expect(markdown).toContain('| official_docs | 22 |');
    expect(markdown).toContain('| public_script | 24 |');
    expect(markdown).toContain('| reduced_fixture_only | 46 |');
    expect(formatPineCompatibilityCoverageJson(index)).toContain('"total": 46');
  });
  ...
  it('generates deterministic dashboard artifacts for CI', () => {
    ...
    try {
      ...
      expect(readFileSync(join(outDir, 'pine-compatibility-corpus.json'), 'utf8')).toContain('"passed": 46');
      expect(readFileSync(join(outDir, 'pine-compatibility-corpus.md'), 'utf8')).toContain('Pass rate: 100.0%');
      expect(readFileSync(join(outDir, 'pine-compatibility-coverage.json'), 'utf8')).toContain('"total": 46');
      expect(readFileSync(join(outDir, 'pine-compatibility-coverage.md'), 'utf8')).toContain('Total checkpoints: 46');
    } finally {
