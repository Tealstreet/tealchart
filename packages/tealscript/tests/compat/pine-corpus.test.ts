...
describe('Pine compatibility checkpoint corpus', () => {
  it('keeps source-linked reduced checkpoints in the offline corpus', () => {
    ...
    expect(compatibilityCheckpointLedger.entries).toHaveLength(64);
    expect(run.summary.total).toBe(64);
    expect(run.summary.passed).toBe(64);
  ...
  it('renders a stable checkpoint corpus report', () => {
    ...
    expect(markdown).toContain('Total: 64');
    ...
    expect(markdown).toContain('| dashboard | 4 | 4 | 0 |');
...
`, {
  ...
  it('builds a checkpoint coverage index from intake metadata', () => {
    ...
    expect(index).toMatchObject({
      schemaVersion: 1,
      total: 64,
      byCategory: { indicator: 43, strategy: 21 },
      bySourceKind: { official_docs: 28, public_script: 36 },
      byPineVersion: { v6: 64 },
      byStoragePolicy: { reduced_fixture_only: 64 },
    });
    ...
    expect(markdown).toContain('Total checkpoints: 64');
    expect(markdown).toContain('| official_docs | 28 |');
    expect(markdown).toContain('| public_script | 36 |');
    expect(markdown).toContain('| reduced_fixture_only | 64 |');
    expect(formatPineCompatibilityCoverageJson(index)).toContain('"total": 64');
  });
  ...
  it('generates deterministic dashboard artifacts for CI', () => {
    ...
    try {
      ...
      expect(readFileSync(join(outDir, 'pine-compatibility-corpus.json'), 'utf8')).toContain('"passed": 64');
      expect(readFileSync(join(outDir, 'pine-compatibility-corpus.md'), 'utf8')).toContain('Pass rate: 100.0%');
      expect(readFileSync(join(outDir, 'pine-compatibility-coverage.json'), 'utf8')).toContain('"total": 64');
      expect(readFileSync(join(outDir, 'pine-compatibility-coverage.md'), 'utf8')).toContain('Total checkpoints: 64');
    } finally {
