...
- `str.format_time()` uses the Pine v6 ISO-style default format and supports
  ...
  `D`/`DD`/`DDD` tokens, week-of-year `w`/`ww` tokens, fractional-second
  `S`/`SS`/`SSS` tokens, 12-hour `h`/`hh` and AM/PM `a` tokens, timezone-name
  `z`/`zzzz` tokens, and numeric timezone offsets.
