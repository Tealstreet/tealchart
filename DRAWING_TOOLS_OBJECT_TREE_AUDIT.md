...
| --- | --- | --- | --- |
| Built-in surface | `TealchartWidget.test.ts` covers the web built-in object-tree fallback when no app-owned callback is provided, including panel pointer/context-event isolation from chart fallthrough. | `UserDrawingObjectTreeSheet.test.tsx` covers the built-in mobile sheet, including sheet responder isolation from chart taps while row/actions are used. | `objectTree.test.ts` covers row/group model generation. |
...
- Built-in web and mobile object-tree surfaces now both have focused regression
  evidence that interacting inside the surface does not leak chart gestures or
  dismiss the mobile sheet before a row/action can complete.
